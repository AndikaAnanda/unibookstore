<div x-ref="editBookModal{{ $book->id_buku }}" class="fixed inset-0 z-10 hidden overflow-y-auto"
    ...
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div
            ...
            <form action="{{ route('admin.buku.update', $book->id_buku) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <h3 class="text-lg font-medium text-gray-900 mb-4">Edit Buku</h3>
                    <div class="mb-4">
                        <label for="id_buku" class="block text-sm font-medium text-gray-700 mb-1">ID Buku</label>
                        <input type="text" value="{{ $book->id_buku }}"
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="nama_buku" value="{{ $book->nama_buku }}"
                            class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500"
                            required>
                    </div>
                    <div class="mb-4">
                    ...
                    <div class="mb-4">
                        ...
                        <select name="id_penerbit"
                            ...
                            @foreach ($publishers as $publisher)
                                <option value="{{ $publisher->id_penerbit }}"
                                    {{ $book->id_penerbit == $publisher->id_penerbit ? 'selected' : '' }}>
                                    {{ $publisher->nama }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                    ...
                    <button type="button"
                        @@click="$refs.editBookModal{{ $book->id_buku }}.classList.add('hidden')"
